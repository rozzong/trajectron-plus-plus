...
class Unicycle(DynamicalModel):
    ...
    def __init__(
        ...
        super().__init__(dt, limits, xz_size)

    def initialize(self, xz_size, *args, **kwargs) -> None:
        self.F_s = torch.eye(4, dtype=torch.float32)
        self.F_s[0:2, 2:] = torch.eye(2, dtype=torch.float32) * self.dt
        self.F_s_t = self.F_s.transpose(-2, -1)
        self.p0_model = nn.Linear(xz_size + 1, 1)
    ...
    def dynamic(self, x, u):
        ...
        d1 = torch.stack(
            [
                x_p + (a / dphi) * dcos_domega + v * dsin_domega
                + (a / dphi) * torch.sin(phi_p_omega_dt) * self.dt,
                y_p - v * dcos_domega + (a / dphi) * dsin_domega
        ...
        d2 = torch.stack(
            [
                x_p + v * torch.cos(phi) * self.dt
                + (a / 2) * torch.cos(phi) * self.dt ** 2,
                y_p + v * torch.sin(phi) * self.dt
    ...
    def integrate_samples(self, control_samples, x=None):
        ...
        phi_0 += torch.tanh(self.p0_model(torch.cat((x, phi_0), dim=-1)))
